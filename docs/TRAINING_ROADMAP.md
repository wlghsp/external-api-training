...
### Phase 5: 훈련용 시뮬레이터를 넘어서 — 실제 결제가 붙은 사이드 프로젝트
**파일**: 미작성
**전제 조건**: Phase 1~4 모두 완료 (실습/블로그/이해도 테스트 통과)
**동기**: Phase 1~4는 `pg-simulator`라는 통제된 환경에서 장애를 인위로 재현하며 패턴을 익히는 훈련이다. 하지만 idempotency, circuit breaker, reconciliation, 커넥션풀 산정, bulkhead 같은 패턴이 진짜 체화되는 시점은 실제 사용자, 실제 PG 샌드박스, 실제 예측 불가능한 실패를 마주할 때다. 이 Phase는 "학습을 끝내는" 단계가 아니라 "지금까지의 학습을 실전 검증하는" 단계로 둔다.

**방향성 (초안, 착수 시점에 구체화)**:
- 형태는 **쇼핑몰**로 잡는다 — 실제 돈이 오가지 않아도 "진짜 결제처럼" 느껴지는 흐름(주문 → 재고 차감 → 결제 → 성공/실패/취소·환불)이 있어야, 만들면서 계속 그 로직을 머릿속에 그리고 몰입하게 된다. 결제만 뚝 떼어 흉내내는 것보다 쇼핑몰이라는 맥락이 있어야 왜 이 패턴이 필요한지 몸으로 느끼기 쉽다.
- 핵심은 **동시성 처리가 실제로 요구되는 도메인**을 고르는 것이다 — 결제 멱등성뿐 아니라 **재고 차감**처럼 "동시에 여러 요청이 같은 자원을 두고 경쟁하는" 문제를 의도적으로 포함시킨다 (예: 한정 수량 상품 동시 주문, 재고 초과 판매 방지).
- `apps/commerce-api`를 기반으로 확장할지, 완전히 새 저장소로 시작할지는 착수 시점에 결정한다 — 기존 도메인 모델(PaymentModel, IdempotencyLock 등)과 인프라 설정을 재사용하면 처음부터 새로 짜는 것보다 학습 곡선이 낮다.
- 실제 PG 샌드박스 연동을 목표로 한다 (토스페이먼츠, 아임포트/포트원, 카카오페이 등 — 국내 서비스 기준 샌드박스가 열려있는 곳 중 선택).
- Phase 1~4에서 배운 패턴을 실전에 그대로 적용하는 것이 목표다 — 새로운 패턴을 더 배우는 것이 아니라, 훈련에서 익힌 것이 실제 트래픽/실제 실패 앞에서도 설명 가능한 형태로 동작하는지 확인한다. 재고 동시성은 Phase 1~4 범위 밖의 새 학습 요소이므로(비관적 락/낙관적 락/분산 락 재적용), 별도 이해도 테스트 없이도 스스로 설계 근거를 설명할 수 있어야 한다는 원칙(`phase1-quiz.md` 방식)은 유지한다.
- 배포까지 포함한다 — 로컬 docker-compose가 아니라 실제로 접근 가능한 환경에 올려서, "운영 중인 결제 코드"라는 조건을 실제로 만족시킨다.

**착수 트리거**: Phase 4 완료 후 이 섹션을 실제 실행 계획(어떤 서비스, 어떤 PG, 어떤 배포 환경)으로 구체화한다.
