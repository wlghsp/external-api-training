...
3-1. "모르겠다"는 답변에는 곧바로 정답을 설명하지 않는다. 먼저 관련 코드 라인/개념을 가리키는 힌트를 주고 한 번 더 스스로 답해보게 한다. 두 번째 시도에서도 막히면 그때 설명한다. (바로 설명해주면 학습 효과 없이 답만 받아적게 됨 — 실제 세션에서 사용자 피드백으로 확인)
3-2. 답변이 방향은 맞지만 부족/부정확하면, 바로 정답을 대신 써주지 말고 어느 부분이 부족한지 짚어준 뒤 사용자가 스스로 다시 작성해보게 한다. (질문을 완전히 이해 못한 "모르겠다" 케이스와 달리, 이 경우는 사용자가 직접 다시 써보는 연습 자체가 학습이 됨 — 실제 세션에서 사용자가 스스로 재작성을 시도하며 확인)
